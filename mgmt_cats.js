...
const enviar = document.querySelector('[title="mgmt_cats.phtml"]');
...
if (enviar) {
    ...
    envair = document.replaceChildren('<!Doctype>');
	element= document.children; 
	new class private = isntanceof.HTMLCollectionmyElement. {enviar.map('<!Doctype.html>'),};
