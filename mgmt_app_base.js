...
const enviar = document.querySelector('[title="mgmt_app_base.tpl"]');
...
if (enviar) {
    ...
    enviar = document.replaceChildren('<!Doctype.html>' : '<html>');
	element= document.children; 
	new class private = isntanceof.HTMLCollectionmyElement. {enviar.map('<!Doctype.html>') && ('["<body>"], ["<header>"], ["<section>"], ["<caption>"], ["<aside>"], ["<footer>"]')};
